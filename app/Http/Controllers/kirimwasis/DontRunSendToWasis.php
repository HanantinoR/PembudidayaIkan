...
$query_ambil_data_survey_belom_dikirim = 'SELECT * FROM survey WHERE response_wasis IS NULL;';
...
if ($hasil_cek_ambil_data_survey_belom_dikirim = mysqli_query($conn, $query_ambil_data_survey_belom_dikirim)) {
    ...
    if ($hasil_cek_ambil_data_survey_belom_dikirim->num_rows === 0) {
        ...
        echo "Per Tanggal ".date('Y-m-d H:i:s').", Data Belum Dikirim Sebanyak ".$hasil_cek_ambil_data_survey_belom_dikirim->num_rows."\n";
        echo "Memulai Mengirim Data Satu-per-satu...\n\n";
        ...
        foreach (mysqli_fetch_all($hasil_cek_ambil_data_survey_belom_dikirim,MYSQLI_ASSOC) as $key => $value) {
            ...
            if ($hasil_cek_get_id_komoditas = mysqli_query($conn, $query_get_id_komoditas)) {
                ...
                if ($hasil_cek_get_id_komoditas->num_rows === 0) {
                    ...
                    echo "Data Disekip Kirim Ke Wasis...\n\n";
                    $jumlah_gagal++;
                                                ...
                                                if ($hasil_cek_get_kabupaten_distribusi->num_rows === 0) {
                                                    ...
                                                    if ($hasil_cek_get_kabupaten_sumber_benih = mysqli_query($conn, $query_get_kabupaten_sumber_benih)) {
                                                        ...
                                                        if ($hasil_cek_get_kabupaten_sumber_benih->num_rows === 0) {
                                                            ...
                                                            $padat_pelihara=$value["padat_tebar"];
                                                            ...
                                                            $tk_num=$value["jumlah_tenaga_kerja"];
                                                            $omzet=$value["modal"];
                                                            ...
                                                            $pakan_num=$value["jumlah_pakan"];
                                                            $pakan_sumber='';
                                                            $pakan_harga=$value["harga_pakan"];
                                                            $benur_sumber='';
                                                            $benur_num=$value["jumlah_benih"];
                                                            $benur_harga=$value["harga_benih"];
                                                            $jarak_tambak=(trim($value["jarak_pantai"]) == 0) ? '' : $value["jarak_pantai"];
                                                            ...
                                                            $family_num=$value["jumlah_keluarga"];
                                                            $biaya_media=$value["biaya_pembuatan_media_pemeliharaan"];
                                                            $biaya_susut=$value["biaya_penyusutan"];
                                                            $biaya_tk=$value["biaya_tenaga_kerja"];
                                                            $income=$value["pendapatan"];
                                                            $biaya_pakan=$value["biaya_pakan"];
                                                            $biaya_benih=$value["biaya_benih"];
                                                            $biaya_alat=$value["biaya_pembelian_peralatan"];
                                                            $photo='';
                                                            ...
                                                            $query_update_data_wasis ='UPDATE survey '
                                                                                ."SET response_wasis = '".trim($response)
